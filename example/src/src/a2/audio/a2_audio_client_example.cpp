...
#include "a2/a2_audio_client.hpp"
...
auto Control(std::shared_ptr<unitree::robot::a2::AudioClient> client,
             const std::string &audio_file_path) {
  /*TTS Example*/
  int32_t ret =
      client->TtsMaker("你好。我是宇树科技的机器人A2。例程启动成功", 0);
...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "Error: Audio file path not provided!\n"
              << "Usage: a2_audio_client_example audio_file_path" << std::endl;
    return 1;
  }
